...
import { 
  Gift, ArrowLeft, Filter, Calendar, Building2, User, 
...
export function FreeQuantityReportPage() {
  ...
  const fetchReport = async () => {
    try {
      ...
      showErrorToast('Failed to load free quantity report');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleExport = () => {
    const headers = ['Date', 'Order No', 'Company', 'Route', 'Shop', 'Delivery Man', 'Product', 'Ordered Qty', 'Free Qty', 'Unit', 'Price', 'Free Value', 'Status'];
    ...
    link.setAttribute("download", `free_quantity_report_${getTodayBDDate()}.csv`);
  ...
  return (
    <div className="space-y-6 pb-24 print:pb-0 print:space-y-4">
      ...
      <div className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between bg-white px-4 py-3 border-b border-slate-100 lg:hidden print:hidden">
        ...
        <h1 className="text-sm font-black uppercase tracking-widest text-slate-900">Free Qty</h1>
      ...
      <div className="hidden lg:flex lg:items-end lg:justify-between pt-4 lg:pt-0 print:block">
        <div>
          ...
          <h1 className="mt-3 text-3xl font-black tracking-tight text-slate-900 print:text-2xl print:mt-1">Free Quantity Report</h1>
          <p className="mt-2 text-sm font-medium text-slate-500 print:hidden">Analyze free product pieces given across all dimensions.</p>
        </div>
      ...
      <div className="grid grid-cols-1 min-[380px]:grid-cols-2 lg:grid-cols-4 gap-4 print:grid-cols-4 print:gap-2">
        <div className="rounded-[1.75rem] border border-slate-100 bg-indigo-50/50 p-5 shadow-sm print:bg-white print:border-slate-200">
          <div className="flex items-center gap-3">
            <div className="p-2 bg-indigo-100 rounded-xl print:hidden"><Gift className="h-4 w-4 text-indigo-600" /></div>
            <div>
              <p className="text-[10px] font-black uppercase tracking-[0.2em] text-indigo-400 leading-none">Total Free Qty</p>
              <h3 className="mt-1 text-lg font-black text-slate-900">{formatNumber(report?.summary?.totalFreeQty || 0)} PCS</h3>
            </div>
          </div>
        </div>
        <div className="rounded-[1.75rem] border border-slate-100 bg-emerald-50/50 p-5 shadow-sm print:bg-white print:border-slate-200">
          <div className="flex items-center gap-3">
            <div className="p-2 bg-emerald-100 rounded-xl print:hidden"><TrendingUp className="h-4 w-4 text-emerald-600" /></div>
            <div>
              <p className="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-400 leading-none">Today Free Qty</p>
              <h3 className="mt-1 text-lg font-black text-slate-900">{formatNumber(report?.summary?.todayFreeQty || 0)} PCS</h3>
            </div>
          </div>
        </div>
        <div className="rounded-[1.75rem] border border-slate-100 bg-rose-50/50 p-5 shadow-sm print:bg-white print:border-slate-200">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-[10px] font-black uppercase tracking-[0.2em] text-rose-400 leading-none">Free Value</p>
              <h3 className="mt-1 text-lg font-black text-rose-700">{formatCurrency(report?.summary?.totalFreeValue || 0)}</h3>
            </div>
          </div>
        </div>
        <div className="rounded-[1.75rem] border border-slate-100 bg-amber-50/50 p-5 shadow-sm print:bg-white print:border-slate-200">
          <div className="flex items-center gap-3">
            <div className="p-2 bg-amber-100 rounded-xl print:hidden"><ShoppingCart className="h-4 w-4 text-amber-600" /></div>
            <div>
              <p className="text-[10px] font-black uppercase tracking-[0.2em] text-amber-400 leading-none">Total Orders</p>
              <h3 className="mt-1 text-lg font-black text-amber-700">{formatNumber(report?.summary?.totalOrders || 0)}</h3>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 print:hidden">
        <div className="bg-white rounded-3xl border border-slate-100 p-4">
           <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-1">Top Free Product</p>
      ...
      <PageCard noPadding className="overflow-hidden print:border-none print:shadow-none">
                ...
                <tbody className="divide-y divide-slate-50">
                  {report?.detailRows.map((r: any) => (
                    <tr key={r.id} className="transition hover:bg-slate-50/60">
                      ...
                      <td className="px-6 py-4 text-xs font-black text-slate-900">#{r.orderId}</td>
                      ...
                      <td className="px-6 py-4 text-center text-xs font-black text-slate-500">{r.orderedQty}</td>
                      <td className="px-6 py-4 text-center text-xs font-black text-indigo-600 bg-indigo-50/30">{r.freeQty}</td>
                      <td className="px-6 py-4 text-right text-xs font-black text-rose-600">{formatCurrency(r.freeValue)}</td>
                      <td className="px-6 py-4 text-center">
                        <span className={`px-2 py-0.5 rounded-full text-[9px] font-black uppercase ${
                          r.status === 'SETTLED' ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-600'
                        }`}>
                          {r.status}
                        </span>
                      </td>
              ...
              {report?.detailRows.map((r: any) => (
                <div key={r.id} className="p-4 space-y-3 active:bg-slate-50 transition-colors">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="px-2 py-0.5 rounded-full text-[9px] font-black uppercase bg-slate-100 text-slate-600">{r.status}</span>
                  </div>
                  <div>
                  ...
                  <div className="flex items-center justify-between pt-2 border-t border-slate-50">
                    <div className="flex items-center gap-4">
                       <div>
                          <p className="text-[9px] font-black text-slate-400 uppercase">Ordered</p>
                          <p className="text-xs font-black text-slate-900">{r.orderedQty}</p>
                       </div>
                       <div>
                          <p className="text-[9px] font-black text-slate-400 uppercase">Free</p>
                          <p className="text-xs font-black text-indigo-600">{r.freeQty}</p>
                       </div>
                    </div>
                    <div className="text-right">
                       <p className="text-[9px] font-black text-slate-400 uppercase">Value</p>
                       <p className="text-xs font-black text-rose-600">{formatCurrency(r.freeValue)}</p>
                    </div>
                  </div>
                </div>
              ))}
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="border-b border-slate-100 bg-slate-50/70 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">
                  ...
                  <th className="px-6 py-4 text-center">Total Free Qty</th>
                  <th className="px-6 py-4 text-right">Total Free Value</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-50">
                {(activeTab === 'company' ? report?.companySummary :
                  ...
                  <tr key={row.id} className="transition hover:bg-slate-50/60">
                    ...
                    <td className="px-6 py-4 text-center text-sm font-black text-indigo-600">{formatNumber(row.totalQty)} PCS</td>
        ...
        {!report?.detailRows?.length && (
          <div className="py-20 text-center">
            <StateMessage
              title="No free quantity records"
              description="No free items were found for the selected filters."
              icon={<Gift className="mx-auto mb-3 h-10 w-10 text-slate-200" />}
            />
          </div>
        )}
      </PageCard>
