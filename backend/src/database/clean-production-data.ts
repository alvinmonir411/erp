...
async function cleanProductionData() {
  if (
    ...
    process.env.CLEAN_CONFIRM !== 'YES'
  ) {
    console.error(
      'ERROR: You are attempting to run a destructive script in production.',
    );
    console.error(
      'To proceed, you must set CLEAN_CONFIRM=YES in your environment variables.',
    );
    process.exit(1);
  }
