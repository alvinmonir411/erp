  ...
  async getDashboardData(companyId?: number, user?: any) {
    ...
    if (companyId) {
      ...
      aggResult = await qb
        ...
        .addSelect('SUM(CASE WHEN order.status <> \'CANCELLED\' THEN COALESCE(item.lineTotal, 0) ELSE 0 END)', 'totalOrderValue')
        .addSelect('COUNT(DISTINCT CASE WHEN order.status = \'CANCELLED\' THEN order.id END)', 'cancelledOrdersCount')
        .addSelect(`SUM(CASE WHEN order.status = 'SETTLED' THEN
        ...
        // Daily (today) metrics
        .addSelect('COUNT(DISTINCT CASE WHEN order.orderDate = :todayDateStr AND order.status <> \'CANCELLED\' THEN order.id END)', 'todayOrdersCount')
        .addSelect('SUM(CASE WHEN order.orderDate = :todayDateStr AND order.status <> \'CANCELLED\' THEN COALESCE(item.lineTotal, 0) ELSE 0 END)', 'todayOrderValue')
        
        .addSelect('COUNT(DISTINCT CASE WHEN order.dispatchedAt >= :todayStartUTC AND order.dispatchedAt <= :todayEndUTC THEN order.id END)', 'todayDispatchCount')
        .addSelect('SUM(CASE WHEN order.dispatchedAt >= :todayStartUTC AND order.dispatchedAt <= :todayEndUTC THEN COALESCE(item.lineTotal, 0) ELSE 0 END)', 'todayDispatchValue')
        .addSelect(`SUM(CASE WHEN order.status = 'SETTLED' AND order.settledAt >= :todayStartUTC AND order.settledAt <= :todayEndUTC THEN
          COALESCE(item.deliveredPaidQuantity, 0) * (
            CASE WHEN COALESCE(item.quantity, 0) > 0 THEN (COALESCE(item.lineTotal, 0) / item.quantity)
            ELSE COALESCE(item.unitPrice, 0) END
          )
        ELSE 0 END)`, 'todaySettledValue')
        .addSelect('COUNT(DISTINCT CASE WHEN order.status = \'CANCELLED\' AND order.updatedAt >= :todayStartUTC AND order.updatedAt <= :todayEndUTC THEN order.id END)', 'todayCancelledCount')
        
        // Delivery status totals
        .addSelect('COUNT(DISTINCT CASE WHEN order.dispatchedAt IS NOT NULL THEN order.id END)', 'totalDispatchCount')
        .addSelect('COUNT(DISTINCT CASE WHEN order.status IN (\'CONFIRMED\', \'ASSIGNED\') THEN order.id END)', 'pendingDispatchCount')
        .addSelect('COUNT(DISTINCT CASE WHEN order.status IN (\'DELIVERED\', \'SETTLED\') THEN order.id END)', 'deliveredCount')
      ...
      aggResult = await baseQb
        ...
        // Daily (today) metrics
        .addSelect(`COUNT(CASE WHEN order.orderDate = :todayDateStr AND order.status <> 'CANCELLED' THEN 1 END)`, 'todayOrdersCount')
        .addSelect(`SUM(CASE WHEN order.orderDate = :todayDateStr AND order.status <> 'CANCELLED' THEN COALESCE(order.grandTotal, 0) ELSE 0 END)`, 'todayOrderValue')
        ...
        .addSelect(`SUM(CASE WHEN order.dispatchedAt >= :todayStartUTC AND order.dispatchedAt <= :todayEndUTC THEN COALESCE(order.grandTotal, 0) ELSE 0 END)`, 'todayDispatchValue')
    ...
    const profitResult = await this.orderItemsRepository.createQueryBuilder('item')
      ...
      .andWhere(companyId ? 'product.companyId = :companyId' : '1=1', { companyId })
    ...
    try {
      ...
      if (companyId) {
        duesQb.innerJoin('order.items', 'item')
              .innerJoin('item.product', 'product')
      ...
      const todayDuesQb = this.duesRepository.createQueryBuilder('due')
        .leftJoin('due.order', 'order')
        .select('SUM(COALESCE(due.dueAmount, 0))', 'todayDue')
        .where('due.createdAt >= :todayStartUTC AND due.createdAt <= :todayEndUTC', { todayStartUTC, todayEndUTC });
      if (companyId) {
        todayDuesQb.innerJoin('order.items', 'item')
                   .innerJoin('item.product', 'product')
                   .andWhere('product.companyId = :companyId', { companyId });
      }
      ...
      if (companyId) {
        collQb.innerJoin('order.items', 'item')
              .innerJoin('item.product', 'product')
              .andWhere('product.companyId = :companyId', { companyId });
      }
      ...
      const todayCollQb = this.collectionsRepository.createQueryBuilder('coll')
        .leftJoin('coll.order', 'order')
        .select('SUM(COALESCE(coll.collectedAmount, 0))', 'todayCollected')
        .where('coll.status = :approved', { approved: CollectionStatus.APPROVED })
        .andWhere('coll.createdAt >= :todayStartUTC AND coll.createdAt <= :todayEndUTC', { todayStartUTC, todayEndUTC });
      if (companyId) {
        todayCollQb.innerJoin('order.items', 'item')
                   .innerJoin('item.product', 'product')
                   .andWhere('product.companyId = :companyId', { companyId });
      }
    ...
    const salesQb = this.orderItemsRepository.createQueryBuilder('item')
      .leftJoin('item.order', 'order')
      .leftJoin('item.product', 'product')
